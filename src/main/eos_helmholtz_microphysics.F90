...
!
!  $Id: 5a8bbfea992364396b7366e26deaef68a87e65c5 $
!
...
!____________AQUI ACABAN LAS NUEVAS DECLARACIONES______________________________

  public :: read_eos_helmholtz
  public :: helmeos
...
!_______________________________________________________________________
 ...
 subroutine helmeos(temp, den, abar, zbar, ierr, pres_opt, sound_opt, ener_opt, cv_opt, dpresdt_opt, denerdt_opt)
...
! declare
      ...
      integer,          intent(out):: ierr
      real, optional,   intent(out):: pres_opt, sound_opt, cv_opt, dpresdt_opt
      real, optional,   intent(out):: ener_opt, denerdt_opt

      double precision             :: pres, sound, ener, cv, dpresdt, denerdt
      
      double precision, parameter :: pi       = 3.1415926535897932384d0, &
                        eulercon = 0.577215664901532861d0, &
                        a2rad    = pi/180.0d0,  rad2a = 180.0d0/pi
      ! physical constants
      double precision, parameter :: g       = 6.6742867d-8, &
                        h       = 6.6260689633d-27, &
                        hbar    = 0.5d0 * h/pi, &
                        qe      = 4.8032042712d-10, &
                        avo     = 6.0221417930d23, &
      ...
      double precision ytot1,ye, &
                       ...
                       sion,xnem,pele,eele,sele,entr,dpresdd, &
                       denerdd,dentrdd,dentrdt,cp, &
                       gam1,gam2,gam3,chit,chid,nabad,etaele, &
...
! bomb proof the input
        ...
        if (din  .lt. d(1)) then
         ...
         print *, 'ye*den too small, off grid'
         print *, temp
         print *, ye
         print *, den
...
!      enddo
      ...
      !print *, gam1
      !print *, chid
      !print *, "dpraddd,dpgasdd,dpiondd,dpepdd,dpcouldd"       
      !print *, dpraddd
      !print *, dpgasdd
      !print *, dpiondd
      !print *, dpepdd
      ...
      if ( present(pres_opt))    pres_opt    = pres
      if ( present(sound_opt))   sound_opt   = sound
      if ( present(cv_opt))      cv_opt      = cv
      if ( present(dpresdt_opt)) dpresdt_opt = dpresdt
      if ( present(ener_opt))    ener_opt    = ener
      if ( present(denerdt_opt)) denerdt_opt = denerdt
