...
!----------------------------------------------------------------
 ...
 use eos_helmholtz,  only:xmass
  ...
  auterm  = mrhoi5*alphau*cvi       !  artificial thermal conductivity parameter
...
ifgas: if (iamgasi .and. iamgasj) then
      ...
      !
        ...
        if (projv < 0.) then
           ...
           dudtdissi = -projv**2*(pmassj*vsigavi*grkerni+pmassj*vsigavj*grkernj)/(rhoi+rhoj) !CHECK
        else
