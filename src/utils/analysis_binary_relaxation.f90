...
subroutine do_analysis(dumpfile,num,xyzh,vxyzu,particlemass,npart,time,iunit)
 ...
 use part,            only: igas,iamtype,iphase,maxphase,rhoh,divcurlv,divcurlB,Bevol,dBevol,dustfrac,ddustfrac, &
                            ipart_rhomax,stressmax
 ...
 real                         :: newseparation,dynfac,dt

 real                         :: fxyzu(4,npart)
divcurlv,divcurlB,Bevol,dBevol,dustfrac,ddustfrac,&
                 ipart_rhomax,,stressmax
...
!
 ...
 real                         :: xcom1(3),xcom2(3),vcom1(3),vcom2(3),dr(3),fcm1(:),fcm2(:)
 logical                      :: iexist
 ...
 newseparation = separation * dynfac * damp * dt
 separationdiff = separation - newseparation
 
 !-- Adjust positions of both stars

 xyzh(1,1:Nstar1)       = xyzh(1,1:Nstar1)       + separationdiff*mstar2/(mstar1+mstar2)
 xyzh(1,Nstar1+1:npart) = xyzh(1,Nstar1+1:npart) - separationdiff*mstar1/(mstar1+mstar2)

 !
 !--- Now with new positions calculate forces on the center of masses so that 
 !    to obtain the exact numerical orbital rotational speed
 !
 
 call force(icall,npart,xyzh,vxyzu,fxyzu,divcurlv,divcurlB,Bevol,dBevol,dustfrac,ddustfrac,&
 ...
 fcm2(:) = 0.
 do i=1,Nstar1
    fcm1(:) = particlemass*fxyzu(1:3,i)
 enddo

 do i=Nstar1+1,npart
    fcm2(:) = particlemass*fxyzu(1:3,i)
 enddo
 
 fcm1(:)=fcm1(:)/mstar1
 fcm2(:)=fcm2(:)/mstar2

 omega1=sqrt(sqrt(fcm1(1)**2+fcm1(2)**2+fcm1(3)**2)/(mstar1*newseparation*mstar2/(mstar1+mstar2)))
 omega2=sqrt(sqrt(fcm2(1)**2+fcm1(2)**2+fcm2(3)**2)/(mstar2*newseparation*mstar1/(mstar1+mstar2)))

 print *, omega1
 print *, omega2

 omega_corotate = omega1
